...
app.put('/api/meals', function (req, res) {
  ...
  let db = pgp(connection_string);
  db.any('INSERT INTO meals(name, description) values($1, $2)', [
    name,
    description,
  ])
    .then((data) => {
...
app.post('/api/meals/:id', function (req, res) {
  ...
  db.tx((t) => {
    ...
    if (
      typeof req.body.pending !== 'undefined' &&
      typeof req.body.pending === 'boolean'
    ) {
      console.log(req.body.pending);
      var meal_pending = t.none('update meals SET pending = $1 WHERE id = $2', [
        req.body.pending,
        id,
      ]);
    ...
    if (req.body.name) {
      var meal_name = t.none('update meals SET name = $1 WHERE id = $2', [
        req.body.name,
        id,
      ]);
    ...
    if (req.body.description) {
      var meal_description = t.none(
        'update meals SET description = $1 WHERE id = $2',
    ...
    var meal_updated = t.none('update meals SET updated = $1 WHERE id = $2', [
      updated,
      id,
    ]);
